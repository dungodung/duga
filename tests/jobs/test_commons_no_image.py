...
from jobs import commons_no_image
...
def entity(has_image=False, label=None, language="sr"):
    claims = {"P18": [{"mainsnak": {}}]} if has_image else {}
    labels = {language: {"value": label, "language": language}} if label is not None else {}
    return {"claims": claims, "labels": labels}


@responses.activate
def test_run_creates_gaps_only_for_non_living_topics_missing_an_image(app):
    with app.app_context():
        ...
        responses.add(
            responses.GET,
            API_URL,
            json={"entities": {"Q1": entity(has_image=False, label="Has no image")}},
            status=200,
        )
        commons_no_image.run(app)

        gaps = Gap.query.filter_by(language_code="sr").all()
        assert {g.topic_qid for g in gaps} == {"Q1"}
        assert gaps[0].project_code == "commons"
        assert gaps[0].gap_type == "no_image"
        assert gaps[0].action_url == "https://www.wikidata.org/wiki/Q1#P18"

        detector = Detector.query.filter_by(detector_key="commons_no_image").first()
...
def test_run_is_idempotent(app):
    with app.app_context():
        ...
        responses.add(
            ...
            json={"entities": {"Q1": entity(has_image=False, label="Missing")}},
            status=200,
        )
        commons_no_image.run(app)
        ...
        responses.add(
            responses.GET, API_URL,
            json={"entities": {"Q1": entity(has_image=True, label="Missing")}},
            status=200,
        )
        commons_no_image.run(app)
