...
from jobs import wikisource_no_text
...
def test_run_creates_gaps_only_for_non_living_topics_missing_the_page(app):
    with app.app_context():
        ...
        wikisource_no_text.run(app)

        gaps = Gap.query.filter_by(language_code="sr").all()
        assert {g.topic_qid for g in gaps} == {"Q1"}
        assert gaps[0].project_code == "wikisource"
        assert gaps[0].gap_type == "no_text"
        assert gaps[0].action_url == "https://www.wikidata.org/wiki/Q1#sitelinks-wikisource"

        detector = Detector.query.filter_by(detector_key="wikisource_no_text").first()
