...
from jobs import wikiquote_no_quotes
...
@responses.activate
def test_run_creates_gaps_only_for_non_living_topics_missing_the_page(app):
    with app.app_context():
        ...
        responses.add(
            responses.GET,
            API_URL,
            json=entities_response({"Q1": entity(sitelinks=[], label="Has no sr page")}),
            status=200,
        )
        wikiquote_no_quotes.run(app)

        gaps = Gap.query.filter_by(language_code="sr").all()
        assert {g.topic_qid for g in gaps} == {"Q1"}
        assert gaps[0].project_code == "wikiquote"
        assert gaps[0].gap_type == "no_quotes"
        assert gaps[0].action_url == "https://www.wikidata.org/wiki/Q1#sitelinks-wikiquote"

        detector = Detector.query.filter_by(detector_key="wikiquote_no_quotes").first()
